...
def create_observability_code_mapper(settings: Any | None = None) -> ObservabilityCodeMapper:
    """Build the effective observability contract registry.

    Compatibility model:
      1. The framework default registry is always loaded by default.
      2. If the embedding agent enables a custom mapping, it overlays the
         framework defaults by canonical key.
      3. Old agents that know nothing about OBSERVABILITY_CODE_MAPPING_* still
         receive the historical GRL/action behavior from the framework default.

    ``OBSERVABILITY_DEFAULT_MAPPING_ENABLED=false`` is an explicit escape hatch
    for deployments that intentionally want no compatibility registry.
    """
    if settings is None:
    ...
    overlay_enabled = bool(getattr(settings, "OBSERVABILITY_CODE_MAPPING_ENABLED", False))
    overlay_path = getattr(settings, "OBSERVABILITY_CODE_MAPPING_PATH", None)
    overlay: dict[str, Any] = {}
    overlay_file: Path | None = None
    if overlay_enabled and overlay_path:
        overlay, overlay_file = _load_mapping_document(overlay_path)

    # Shallow merge is intentional: an agent entry completely overrides the
    # framework entry with the same canonical name, while unspecified defaults
    # remain available. The mapper is rebuilt once so aliases from replaced
    # default entries cannot leak into the effective lookup table.
    ...
    logger.info(
        ...
        "default_entries=%d overlay_enabled=%s overlay_path=%s overlay_entries=%d effective_entries=%d",
        default_enabled,
